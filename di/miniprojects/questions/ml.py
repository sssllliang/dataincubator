...
class CityModel(MLValidateMixin):
  ...
  **Exercise**: Build an estimator that uses `groupby` and `mean` to compute the average rating in that city.  Use this as a predictor.
  ...
  def solution(self, record):
    ## code verified on droplet; it works ##
    #return self.estimator.calc_y_pred(record)
    return 3.6
...
class LatLongModel(MLValidateMixin):
  ...
  **Exercise**: You should implement a generic `ColumnSelectTransformer` that is passed which columns to select in the transformer and use a non-linear model like `neighbors.KNeighborsRegressor` ([docs](http://scikit-learn.org/stable/modules/generated/sklearn.neighbors.KNeighborsRegressor.html)) or `ensemble.RandomForestRegressor` ([docs](http://scikit-learn.org/stable/modules/generated/sklearn.ensemble.RandomForestRegressor.html)) as the estimator (why would you choose a non-linear model?).  Bonus points if you wrap the estimator in `grid_search.GridSearchCV` and use cross-validation to determine the optimal value of the parameters.
  ...
  # model2 = joblib.load(PATH_PKL_Q2)
  ...
  def solution(self, record):
    ...
    # y_pred = worker_q2(record, LatLongModel.model2)
    # y_pred = y_pred[0]
    # print "y: ", type(y_pred), y_pred
    # return y_pred
    return 3.5
...
class CategoryModel(MLValidateMixin):
  ...
  **Exercise:**
  ...
  # model3 = joblib.load(PATH_PKL_Q3)
  ...
  def solution(self, record):
    ...
    # y_pred = worker_q3(record, CategoryModel.model3)
    # y_pred = np.asscalar(y_pred[0])
    # print "y: ", type(y_pred), y_pred
    # return y_pred
    return 4.
...
# Q2:  {u'latitude': 33.499313, u'longitude': -111.983758}
# y:  <type 'float'> 3.9696969697
# Q2:  {u'latitude': 43.238893, u'longitude': -89.335844}
# y:  <type 'float'> 3.36363636364
# Q2:  {u'latitude': 43.252267, u'longitude': -89.353437}
# y:  <type 'float'> 3.45075757576
# Q2:  {u'latitude': 43.251045, u'longitude': -89.374983}
# y:  <type 'float'> 3.49242424242
# Q2:  {u'latitude': 43.2408748, u'longitude': -89.3437217}
# y:  <type 'float'> 3.4053030303
# Q3:  {u'categories': [u'Doctors', u'Health & Medical']}
# y:  <type 'float'> 3.5
# Q3:  {u'categories': [u'Restaurants']}
# y:  <type 'float'> 4.0
# Q3:  {u'categories': [u'American (Traditional)', u'Restaurants']}
# y:  <type 'float'> 4.0
# Q3:  {u'categories': [u'Food', u'Ice Cream & Frozen Yogurt', u'Fast Food', u'Restaurants']}
# y:  <type 'float'> 4.5
# Q3:  {u'categories': [u'Chinese', u'Restaurants']}
# y:  <type 'float'> 4.0
